...
- `REQUEST_DETECT_FIRE_EXTINGUISHER`：请求 Orange Pi 主动识别灭火器，payload 为 `{ "target": "fire_extinguisher", "upload_mode": "event" }`。
- `REQUEST_DETECT_DRONE`：请求 Orange Pi 主动识别无人机，payload 为 `{ "target": "drone", "upload_mode": "event" }`。
...
返回每台设备最新一帧，用于前端实时展示。该接口读取的是覆盖式 latest，不表示历史。
...
- `face_result_json`：可选，人脸识别结果 JSON 字符串。
- `access_decision`：可选，`allow`、`deny` 或 `unknown`。
- `source`：可选，`auto_face`、`fire_extinguisher`、`drone`。
- `mode`：可选，`latest` 或 `event`。
...
1. 覆盖 `/uploads/latest/{device_id}.jpg`，供前端即时刷新。
2. 覆盖 `latest_results` 中该设备最新识别结果。
3. 生成事件 key，例如 `face:person:person_001`、`face:unknown`、`active:drone:drone`。
4. 同一个事件 key 5 秒内不重复写入历史。
5. 通过冷却后，才复制一份图片到 `/uploads/photos/` 并写入历史。
6. 如需要门禁联动，只在通过冷却的授权人脸事件上生成 `OPEN_DOOR`，避免 0.5 秒识别循环刷爆命令队列。
