...
| 成员 | 主责 | 交付物 | 备份方向 |
| --- | --- | --- | --- |
| 高镜涵 | 硬件接入与设备网关 | 设备协议、串口/HTTP/MQTT 接入、遥控命令下发、硬件联调记录 | 后端接口联调 |
| 杨济纶 | 后端 API 与数据库 | REST API、数据库表、历史查询、命令队列、文件存储 | 前端数据接口 |
| 董志新 | GUI 前端与数据可视化 | 仪表盘、历史查询页面、远程控制页面、识别结果展示 | 演示材料与 UI 测试 |
| 李韶庸 | YOLO 目标识别 | 图片上传后的目标识别流程、识别结果结构化输出、灯泡检测扩展 | 人脸识别 |
| 汪任城 | 人脸识别与集成测试 | 授权人员录入、2 真 1 假门禁验证、端到端测试脚本、验收 checklist | YOLO 与报告材料 |
...
- 前端：Web 页面，建议 Vue/React/普通 HTML 均可，重点是可演示。
- 后端：Python FastAPI 或 Node.js Express。
- 数据库：SQLite 起步，阶段二需要多人部署时再换 MySQL/PostgreSQL。
- 图像识别：Python 服务内调用 YOLO；如果来不及部署模型，先保留接口并用 mock 结果联调。
- 人脸识别：本地注册 2 个授权人员，另设 1 个未授权样本；演示时输出 allow/deny。
- 板端部署：使用 Orange Pi / 华为开发板作为设备网关或边缘节点，默认设备 ID 为 `orange-pi-main`。
- 硬件接入：优先让开发板通过 HTTP 访问后端；如果硬件只支持串口，则由高镜涵负责写一个串口转 HTTP 的网关程序。
...
硬件组与软件组只需要围绕 3 类接口对齐：状态上报、图片上传、命令拉取/确认。Orange Pi / 华为开发板建议作为统一网关，负责把传感器、摄像头和执行机构的数据转换成下面的 HTTP 接口。
...
6. 如 YOLO 检测到 `light bulb`，可自动生成 `SET_LIGHT` 命令作为创新功能。
...
| 模块 | 输入 | 输出 | 负责人 |
| --- | --- | --- | --- |
| 设备网关 | 硬件串口/HTTP 数据 | `/api/device/*` 请求、命令执行结果 | 高镜涵 |
| 后端 API | 前端请求、设备请求、识别服务结果 | 数据库记录、JSON 响应、命令队列 | 杨济纶 |
| 前端 GUI | `/api/*` JSON | 页面展示、远程控制请求 | 董志新 |
| YOLO 服务 | 图片路径 | `[{label, confidence}]` | 李韶庸 |
| 人脸识别服务 | 图片路径、人员样本库 | `allow/deny/unknown` 与匹配人员 | 汪任城 |
...
- [ ] 设备网关能上传至少 5 条状态数据。
- [ ] 摄像头图片或本地模拟图片能上传并保存。
- [ ] YOLO 能识别至少一种目标，例如 person 或 light bulb。
...
| YOLO 模型运行慢 | 阶段一先识别固定测试图，或使用轻量模型 |
